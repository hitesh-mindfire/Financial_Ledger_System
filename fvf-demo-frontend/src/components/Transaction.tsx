...
const TransactionList: React.FC = () => {
  const [transactions, setTransactions] = useState<
    TransactionUpdateResponse.AsObject[]
  >([]);
  ...
  const fetchTransactions = async () => {
    ...
    try {
      ...
      setTransactions(
        response.getTransactionsList().map((txn) => txn.toObject())
      );
    } catch (err) {
              ...
              <table className="min-w-full">
                <thead>
                  <tr className="border-b">
                    <th className="py-3 px-4 text-left text-sm font-medium text-gray-600 w-32">
                      Date
                    </th>
                    <th className="py-3 px-4 text-left text-sm font-medium text-gray-600 w-1/2">
                      Card Id
                    </th>
                    <th className="py-3 px-4 text-left text-sm font-medium text-gray-600 w-32">
                      Amount
                ...
                {transactions.length > 0 ? (
                  transactions.map((txn) => (
                    <tr key={txn.id} className="border-b hover:bg-gray-50">
                      ...
                      <td
                        className={`py-4 px-4 text-sm font-medium ${getAmountColor(
                          txn.type
                        )}`}
                      >
                        {txn.amount}
                      ...
                      <td
                        className={`py-4 px-4 text-sm font-medium ${getStatusColor(
                          txn.status
                        )}`}
                      >
                        {txn.status}
